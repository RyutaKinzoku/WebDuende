...
export default class GestorCompromisos extends GestorDB{
    async modificarEntrega(entrega){
        ...
        return axios.post('/api/modificarEntrega',values);
    };
    ...
    async modificarCurso(curso){
        ...
        return axios.post('/api/modificarCurso',values);
    };
    ...
    async modificarCita(cita){
        ...
        return axios.post('/api/modificarCita',values);
    };
    ...
    async eliminar(type, idCompromiso){
        ...
        if(type === "Curso"){
            return axios.post('/api/eliminarCurso',values);
        } else if(type === "Cita") {
            return axios.post('/api/eliminarCita',values);
        } else if(type === "Entrega") {
            return axios.post('/api/eliminarEntrega',values);
        }
    };

    async obtener(type, idCompromiso){
        if(type === "Curso"){
            var response = await axios.get('/api/obtenerCurso',{params: {idCompromiso: idCompromiso} });
            ...
            var response = await axios.get('/api/obtenerCita',{params: {idCompromiso: idCompromiso} });
            if(response.data.length>0){
            ...
            var response = await axios.get('/api/obtenerEntrega',{params: {idCompromiso: idCompromiso} });
            if(response.data.length>0){
    ...
    async agregarEntrega(entrega){
        ...
        return axios.post('/api/agregarEntrega',values);
    };
    ...
    async agregarCurso(curso){
        ...
        return axios.post('/api/agregarCurso',values);
    };
    ...
    async agregarCita(cita){
        ...
        return axios.post('/api/agregarCita',values);
    };

    async obtenerLista(){
        let cursos = await axios.get('/api/getCursos');
        let citas = await axios.get('/api/getCitas');
        let entregas = await axios.get('/api/getEntregas');
        let compromisos = [];
    ...
    async getNext(){
        let valor = await axios.get('/api/getNextCompromisos');
