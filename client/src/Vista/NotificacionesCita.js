...
export default class NotificacionesCita extends Component{
    ...
    obtenerNotificaciones = async() => {
        ...
        let response = await controladora.obtenerNotificacionesCita();
        let notificaciones = [];
        for(let i = 0;i<response.length;i++){
            if(Number(response[i].vista) === 1){
                let publicacion = await controladora.obtenerPublicacion(response[i].idPublicacion);
                response[i].idPublicacion = publicacion.data[0];
    ...
    eliminarNotificacion = async (type, id)=>{
        ...
        if(!response.data){
            swal("Notificación escondida", "", "success").then((value)=>{
                window.location.href='/NotificacionesCita'
            })
        } else {
